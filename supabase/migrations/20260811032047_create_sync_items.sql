...
create policy "read own sync items" on public.sync_items for select to authenticated
using ((select auth.uid()) = user_id);

drop policy if exists "insert own sync items" on public.sync_items;
create policy "insert own sync items" on public.sync_items for insert to authenticated
with check ((select auth.uid()) = user_id);

drop policy if exists "update own sync items" on public.sync_items;
create policy "update own sync items" on public.sync_items for update to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);
...
create trigger sync_items_keep_newest before update on public.sync_items
