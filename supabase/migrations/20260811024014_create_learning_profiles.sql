...
create policy "read own learning profile" on public.learning_profiles for select to authenticated
using ((select auth.uid()) = user_id);

drop policy if exists "create own learning profile" on public.learning_profiles;
create policy "create own learning profile" on public.learning_profiles for insert to authenticated
with check ((select auth.uid()) = user_id);

drop policy if exists "update own learning profile" on public.learning_profiles;
create policy "update own learning profile" on public.learning_profiles for update to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);
